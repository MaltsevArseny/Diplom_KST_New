...
### Добавлено
- Добавлен отдельный статус заказа `Выдан` между `Доставлен` и `Завершён`.
- Выдача заказа администратором теперь переводит доставленный заказ в `Выдан`, а повторная выдача заказов в статусах `Выдан` и `Завершён` блокируется.

### Изменено
- Иконка действия выдачи в таблице заказов заменена на **🤝**, чтобы визуально отражать передачу заказа клиенту.
- Справочник `OrderStatuses` расширен до 9 записей; сидирование обновляет порядок статусов в существующих БД.

### Документация и тесты
- Обновлены `README.md`, `UserManual.md`, `AdminManual.md`, `DATABASE_SCHEMA.md`, техническое задание, пояснительная записка и дипломный DOCX.
- Итого: **311 тестов / 32 класса**.
...
### Изменено
- Выдача заказа администратором теперь разрешена только из статуса `Доставлен`.
- Для любых других статусов `OrderService.issueOrderByReceiptCode` возвращает ошибку `Заказ можно выдать только в статусе «Доставлен»`.
- Отдельная панель «Выдача заказа» удалена: поиск по коду встроен в общий поиск заказов, а выдача выполняется кнопкой **🤝** в столбце **«Действия»**.

### Документация и тесты
- Обновлены `README.md`, `AdminManual.md`, `DATABASE_SCHEMA.md`, техническое задание, пояснительная записка и дипломный DOCX.
- Добавлен тест `OrderServiceTest.issueOrderBeforeDeliveredReturnsError`.
- Итого: **311 тестов / 32 класса**.
...
### Добавлено
- В админском разделе **«Заказы»** общий поиск поддерживает цифровой код и данные штрих-кода, а в столбце **«Действия»** добавлена кнопка **🤝** для выдачи найденного заказа.
- Кнопка **🤝** переводит доставленный заказ в статус `Выдан` и фиксирует операцию в `OrderStatusHistory`.
- `OrderService.findOrderByReceiptCode` и `OrderService.issueOrderByReceiptCode` — сервисные admin-only методы для поиска и выдачи по данным получения.

### Ограничения
- Отменённый заказ нельзя выдать.
- Уже выданный или завершённый заказ не выдаётся повторно.
- Операция доступна только роли `ADMIN`; проверка выполнена в service-слое.

### Документация и тесты
- Обновлены `README.md`, `AdminManual.md`, `DATABASE_SCHEMA.md`, техническое задание, пояснительная записка и дипломный DOCX.
- Добавлены 4 теста `OrderServiceTest` для поиска, выдачи, ошибки несуществующего кода и запрета выдачи покупателем.
- Итого: **310 тестов / 32 класса**.
...
### Добавлено
- После успешного нажатия **«Подтвердить заказ»** открывается модальное окно с данными для получения: штрих-код Code 128-C, восьмизначный цифровой код, дата оформления `dd.MM.yyyy HH:mm` и итоговая сумма.
- В разделе **«Мои заказы»** у каждой карточки появился сворачиваемый блок «Данные для получения» с тем же штрих-кодом, кодом, датой и суммой.
- `OrderReceiptService` — единая утилита для формирования кода заказа, форматирования даты и расчёта sequence Code 128-C.
- JavaFX-компоненты `Code128Barcode` и `OrderReceiptPane` для переиспользования receipt-блока в модальном окне и личном кабинете.

### Архитектура
- Новые поля в БД не потребовались: код получения детерминированно формируется из `Orders.id`, а дата и сумма уже хранятся в `Orders.order_date` и `Orders.total_amount`. Схема остаётся в 3НФ, миграция существующих БД не нужна.

### Документация и тесты
- Обновлены `README.md`, `UserManual.md`, `DATABASE_SCHEMA.md`, техническое задание и пояснительная записка.
- Добавлены 4 теста `OrderReceiptServiceTest`.
- Итого: **306 тестов / 32 класса**.
...
### Добавлено
- Нормализация базы данных до **3НФ**: справочники `Categories` (11 записей) и `OrderStatuses` (9 записей) вынесены в отдельные таблицы с внешними ключами
